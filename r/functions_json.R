...
# 
get.TS.func <- function(ts.df.in){
  ts.sub.df <- data.frame(Longitude = ts.df.in[3],
                          Latitude = ts.df.in[4],
                          # LeafN = ts.df.in[6],
                          Leaf15N = ts.df.in[6])
  
  j.vec.temp <- gsub("'",'"',ts.df.in[7])
  ...
  if(!is.null(tmp.json$id[1])){
    ...
    if(nrow(land.sat.df)>= 1){
      land.sat.df$lon <-rep(ts.df.in[3],nrow(land.sat.df))
      land.sat.df$lat <- rep(ts.df.in[4],nrow(land.sat.df))
      land.sat.df$Leaf15N <- rep(ts.df.in[6],nrow(land.sat.df))
      land.sat.df$ndvi <- get.ndvi.func(nir.in = land.sat.df$nir,
                                        red.in = land.sat.df$red)
      land.sat.df$date.obs <- rep(ts.df.in[5],nrow(land.sat.df))
