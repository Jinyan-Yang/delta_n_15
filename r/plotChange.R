...
change.3.df <- change.3.df[!year(change.3.df$date.change) %in% 2012:2013,]
...
# ####
change.df <- do.call(rbind,change.3.ls)
change.df <- change.df[year(change.df$date.change) %in% 2012:2013,]
tiff('figures/mapChange2012.tif',width = 800,height = 400)
par(mar=c(3,1,1,1))
map('world',col='grey40')
sites <- change.df[,c("lon",'lat')]
sites <- sites[!duplicated(sites),]
