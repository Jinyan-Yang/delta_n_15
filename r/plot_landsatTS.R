...
# plot.df$date <- as.Date(strptime(substr(plot.df$date,15,22),'%Y%m%d',tz = "GMT"))
...
for (i in 1:nrow(landsat.df)) {
  ...
  if(x.df$slope.p>0.05){
    ...
    if(x.df$slope.fit<0){
      col.plot = rgb(0.9,0.1,0.1,0.5)
      pch.plot = 16
    }else{
      col.plot = rgb(0.1,0.1,0.9,0.5)
      pch.plot = 17
    }
  }
  point.size <- min(0.5+abs(x.df$slope.fit)/0.00013,3)
