...
png('figures/slope.lm.png',width=800,height = 800)
...
smoothScatter(all.df.biome[,c("slope.ndvi","slope.fit")],
              ...
              ylab = expression(delta^15*N~('‰'~yr^-1)),
              pch = '')
...
for (i.plot in seq_along(pft.chosen.vec)) {
  ...
  points(x = plot.df$slope.ndvi,y = fit.tmp$fitted.values,col=i.plot,lwd=2,lty=lty.in,type='l')
