...
landsat.g.ts.df.n <- readRDS('cache/ls.n.annual.ts.rds')

landsat.g.ts.df.n$leafN.pred <- exp(landsat.g.ts.df.n$dn15.pred)
hist(landsat.g.ts.df.n$leafN.pred)
...
####
n.sum.site.cln <- readRDS('cache/nMeanGlobal.rds')
n.sum.site.cln$Mean[n.sum.site.cln$Mean>25] <- 25
n.sum.site.cln$Mean[n.sum.site.cln$Mean< 15] <- 15

length(n.sum.site.cln$Mean[n.sum.site.cln$Mean>25])
length(n.sum.site.cln$Mean[n.sum.site.cln$Mean< 5])
#####
n.mean.ra <- (rast(n.sum.site.cln[,c('x','y',"Mean")]))
n.sd.ra <- rast(n.sum.site.cln[,c('x','y',"SD")])
# plot(n.mean.ra)
# plot(n.sd.ra)
...
plot1 <-  p + 
  ...
  scale_fill_gradientn(colours = rev(hcl.colors(5)),
                       breaks = seq(15,25,by=2),
                       na.value = NA,
                       labels = seq(15,25,by=2),
                       name = "Foliar [N] (mg/g)") +
